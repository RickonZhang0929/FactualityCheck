...
class knowledge_qa_pipeline(pipeline):
    ...
    # async def run_for_dataset(self, claims_in_responses, evidences_in_responses):
    #     # claims_in_responses = await self._claim_extraction(responses)
    #     # queries_in_responses = []
    #     # evidences_in_responses = []
    #     # sources_in_responses = []
    ...
    async def verification_only(self, claims, evidences):
        """
            ...
            """
        # print("开始执行验证步骤，传入的声明列表:", claims, "证据列表:", evidences)

        # 确保 claims 和 evidences 长度一致
        if len(claims) != len(evidences):
            raise ValueError("声明列表和证据列表长度不匹配。")

        # 使用 zip() 配对声明和证据，形成元组列表 [(claim, evidence), ...]
        claims_and_evidences = zip(claims, evidences)

        # 为每对声明和证据调用 _verification 方法
        verifications = []
        for claim, evidence in claims_and_evidences:
            print("开始执行单元验证步骤，传入的声明列表:", claim, "证据列表:", evidence)
            # 将字符串形式的 claim 转换为符合要求的字典形式
            claim_dict = {'claim': claim}
            verification_result = await self._verification([claim_dict], [evidence])
            verifications.append(verification_result)
        print("验证结果:", verifications)
    ...
    async def run_with_tool_dataset(self, annotated_dataset_path: str, with_tool_classified_dataset_path: str,
                                    rerun: bool = False, rerun_indices: list = []):
    ...
    async def run_self_check_dataset(self, annotated_dataset_path: str, self_check_classified_dataset_path: str,
                                     fewshot: bool = False, rerun: bool = False, rerun_indices: list = []):

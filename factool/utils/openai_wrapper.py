...
class OpenAIChat():
    def __init__(
        ...
        self.client = OpenAI(
            ...
            api_key="sk-lH0YfSFt0PrQMSBHTTjM3V03qgGTf1oXAMnG2Mfwkmckuqdl",
    ...
    async def _request_with_openai(self, messages, retry=3):
        try:
            response = self.client.with_options(max_retries=retry).chat.completions.create(
                model=self.config['model_name'],
                messages=messages,
                max_tokens=self.config['max_tokens'],
                temperature=self.config['temperature'],
    ...
    async def dispatch_openai_requests(
        ...
        async_responses = [
            self._request_with_openai(messages)
            for messages in messages_list
        ]
